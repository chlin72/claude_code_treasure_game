...
import { getSession, signOut, saveScore } from './lib/auth';
...
export default function App() {
  ...
  // Initialize game and rehydrate session on mount
  useEffect(() => {
    initializeGame();
    setCurrentUser(getSession());
  }, []);

  // Plays the appropriate sound based on whether the box has treasure or a skeleton
  ...
  // Handles successful auth, updates session state and closes the auth modal.
  // Input: session (AuthSession) — newly created session. Output: void.
  const handleAuthSuccess = (session: AuthSession): void => {
    setCurrentUser(session);
    setShowAuthModal(false);
    toast.success(`Welcome, ${session.username}!`);
  };

  // Clears the active session and resets currentUser state.
    ...
    <div className="relative min-h-screen bg-gradient-to-b from-amber-50 to-amber-100 flex flex-col items-center justify-center p-8">
      <div className="absolute top-4 right-4 flex items-center gap-2">
        {currentUser ? (
          <>
            ...
            <Button variant="outline" size="sm" onClick={() => setShowScoreHistory(true)}>
              My Scores
            </Button>
      ...
      <div className="mb-8">
        <div className="text-2xl text-center p-4 bg-amber-200/80 backdrop-blur-sm rounded-lg shadow-lg border-2 border-amber-400">
          ...
          {gameEnded && (
            <div className={`text-xl font-bold mt-1 ${score > 0 ? 'text-green-600' : 'text-amber-700'}`}>
              {score > 0 ? '🎉 You Win!' : "🤝 It's a Tie!"}
            </div>
          )}
        </div>
      </div>
      ...
      />
      <ScoreHistory
        open={showScoreHistory}
        onOpenChange={setShowScoreHistory}
